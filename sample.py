...
def sample_multiple_grasps(number_of_candidates, mesh, gripper_name, systematic_sampling,
    ...
    # (+) распаковываем кортеж, берём только список булей
...
def make_parser():
    ...
    parser.add_argument('--object_file', type=str,
                        default='/home/danil/mesh/go.stl',
    ...
    if args.add_quality_metric:
        ...
        points, normals, transforms, roll_angles, standoffs, collisions, qualities\
            = sample_multiple_grasps(args.num_samples,
                                     ...
                                     #(1+) убрал строчку (такого аргумента нет в grasp_estimator.py) filter_best_per_position=args.filter_best_per_position,
        ...
        #(1+) дообавил цикл преобразования (5 строк ниже) 
        for k, v in grasps.items():
            if isinstance(v, list):
                grasps[k] = [float(x) if hasattr(x, 'item') else x for x in v]
            elif hasattr(v, 'item'):
                grasps[k] = v.item()
